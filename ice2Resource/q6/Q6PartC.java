...
public class Q6PartC {
    ...
    public static String getFarmerDetails(CommunityGarden garden, int plotNumber) {
        ...
        Plot currentPlot = garden.getPlot(plotNumber);
        ...
        if (currentPlot.getFarmer() != null) {
            String farmerName = currentPlot.getFarmer().getName();
            String farmerGender = currentPlot.getFarmer().getGender() == 'F' ? "Female" : "Male";
            return String.format("%s[%s]", farmerName, farmerGender);
        } else {
            return "available";
        }
    }
