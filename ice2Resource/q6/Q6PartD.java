...
public class Q6PartD {
    ...
    public static double getPercentageOfLeasedPlotsWithCrop(CommunityGarden garden, char category) {
        ...
        double numLeasedPlots = 0;
        double numCropPlots = 0;
        for (int i = 1; i <= garden.getTotalPlots(); i++){
            if (garden.getPlot(i).getFarmer() != null){
                numLeasedPlots++;
                if (garden.getPlot(i).getCrop() != null && garden.getPlot(i).getCrop().getCategory() == category){
                    numCropPlots++;
                }
            }
        }
        return (numCropPlots / numLeasedPlots) * 100;
    }
}